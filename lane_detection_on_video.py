...
def process(img):
    height = img.shape[0]
    width = img.shape[1]
    region_of_interest_vertices = [(width/8, height), (width/2,height/2),(width,height)]
    ...
    lines = cv2.HoughLinesP(cropped_image, 6, np.pi/60, 160, np.array([]), minLineLength=50, maxLineGap=25)
    image_with_lines = draw_lines(img, lines)
    return image_with_lines

cap = cv2.VideoCapture('media/testvideo2.mp4')
